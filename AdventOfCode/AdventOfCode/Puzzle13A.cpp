...
namespace Puzzle13A
{
    ...
    void PrintSolution()
    {
        ...
        auto pairIndex = 1;
        auto sortedIndexSum = 0;
        for (const auto& chunk : input | ranges::views::chunk(3))
        {
            auto left = ParseEntry(chunk[0]);
            auto right = ParseEntry(chunk[1]);
            if (left <= right)
            {
                sortedIndexSum += pairIndex;
            }
            ++pairIndex;
        }

        std::cout << sortedIndexSum;
    }
}