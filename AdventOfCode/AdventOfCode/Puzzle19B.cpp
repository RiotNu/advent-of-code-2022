...
namespace Puzzle19B
{
	...
	void PrintSolution()
	{
		...
		auto input = Utilities::ReadAllLinesInFile("Puzzle19.input");
		for (auto i = 0; i < 3; ++i)
		{
		...
		std::cout << ranges::accumulate(blueprints, 1, [](int sum, const Blueprint& blueprint) { return sum * GetMaxGeodes(blueprint); });
	}
}