...
IMPLEMENT_STD_HASH(Puzzle24B::Vec2, value.x, value.y);
...
namespace Puzzle24B
{
	...
	auto BreadthFirstSearch(const auto& blizzardStates, const auto& source, const auto& target, const auto& initialHistory)
	{
		...
		explorationQueue.emplace(source, initialHistory);
	...
	{
		auto initialHistory = std::vector<Vec2>{ source };
		return BreadthFirstSearch(blizzardStates, source, target, initialHistory);
	}
	...
	void PrintSolution()
	{
		...
		bestPath = BreadthFirstSearch(blizzardStates, target, source, bestPath);
		bestPath = BreadthFirstSearch(blizzardStates, source, target, bestPath);
