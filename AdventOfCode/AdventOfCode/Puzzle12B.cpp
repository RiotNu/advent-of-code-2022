...
namespace Puzzle12B
{
	...
	void PrintSolution()
	{
		...
		for (auto y = 0; y < grid.Height(); ++y)
		{
			...
			{
				// Reverse all edges from part 1 by flipping source and target when adding edges.
				auto source = grid.GetIndexFromCoordinates(x, y);
				if (HasEdge(grid, x, y, x - 1, y))
				{
					auto target = grid.GetIndexFromCoordinates(x - 1, y);
					boost::add_edge(target, source, weight, graph);
				}
		...
		auto minDistance = static_cast<int>(grid.size());
